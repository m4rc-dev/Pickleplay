...
import { Send, ArrowLeft, MessageCircle, Lock, UserPlus, CheckCircle } from 'lucide-react';
...
export const ChatArea: React.FC<ChatAreaProps> = ({
  ...
  // Detect if this is a "message request" — locked but already has messages
  const hasIncomingMessages = messages.some(m => m.sender_id !== currentUserId);
  const isMessageRequest = canMessage === false && hasIncomingMessages;
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 min-w-0">
      ...
      {canMessage === false ? (
        isMessageRequest ? (
          /* Message Request banner — has incoming messages, offer Accept & Reply */
          <div className="bg-white border-t border-slate-100 px-4 py-4 shrink-0">
            <div className="bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3 flex flex-col gap-2.5">
              ...
              <p className="text-[11px] text-amber-700 leading-relaxed">
                <span className="font-semibold">{otherName}</span> sent you a message. Accept to start chatting — they'll be notified you follow them.
              </p>
              <div className="flex gap-2 mt-0.5">
                <button
                  onClick={async () => {
                    ...
                    try { await onAcceptRequest(); } finally { setIsAcceptingRequest(false); }
                  }}
          ...
          /* Pure lock — no mutual follow, no messages yet */
          <div className="bg-white border-t border-slate-100 px-4 py-4 shrink-0">
