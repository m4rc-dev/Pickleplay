...
const TournamentDetail: React.FC<Props> = ({ tournamentId, isOpen, onClose, isOwnerView = false, onManage }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      // Fetch player ratings for participant cards
      try {
                ...
                <div className="space-y-3 animate-in fade-in duration-200">
                  ...
                  {(regMode === 'squad' || regMode === 'both') && (
                    <div className="space-y-2">
                      ...
                      <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 text-sm text-slate-500 font-bold">
                        Squad-based signups are enabled. Squad roster details will be visible once teams register.
                      </div>
                    </div>
                  )}
                        ...
                        .map((reg, i) => {
                          ...
                          return (
                            <div key={reg.id} className={`flex items-center gap-4 rounded-2xl p-4 transition-colors ${
                              isYou ? 'bg-indigo-50 border-2 border-indigo-200' : 'bg-slate-50 border border-slate-100'
                            }`}>
                              {/* Avatar / number */}
